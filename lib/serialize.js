  ...
  conn.sendMessage = async (jid, content, options = {}) => {
    try {
      ...
      if(content.image && options && options.adReply){
      	  let a = await tool.getBuffer(options.adReply.img);
			let b = await tool.getBuffer(content.image.url);
			let c = content.image;
			let d = options.adReply.img
			let e = content.image.url
			let f = await tool.telegraph(content.image)

			let result = a ? a : b ? b : c;
			let result2 = d ? d : e ? e : f;
			//console.log(result);
          ...
          content.contextInfo = {
              externalAdReply: {
                  ...
                  mediaType: options.adReply.type ? options.adReply.type : 1,
                  body: options.adReply.body ? options.adReply.body : 'Whatsapp Bot Multi-Device',
                  thumbnail: tbuff,
                  thumbnailUrl: thumb,
                  renderLargerThumbnail: options.adReply.large ? options.adReply.large : true,
                  showAdAttribution: true,
                  sourceUrl: options.adReply.url ? options.adReply.url : 'https://chat.whatsapp.com/CGMJD56YU2w4v1q5kvuyKg',
                  mediaUrl: options.adReply.mediaUrl ? options.adReply.mediaUrl : 'https://www.instagram.com/p/CT4TopcP_JD/?igshid=NTc4MTIwNjQ2YQ==',
              }
          }
